...
const SearchModal: React.FC<SearchModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-start justify-center p-4 pt-8 sm:pt-20" dir="rtl">
      <div className="bg-white dark:bg-gray-900 rounded-xl sm:rounded-2xl shadow-2xl max-w-4xl w-full max-h-[85vh] sm:max-h-[80vh] overflow-hidden">
        {/* Header */}
        ...
        <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pr-10 pl-4 py-3 border border-emerald-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 bg-white dark:bg-gray-800 dark:text-white text-base sm:text-lg"
              dir="rtl"
            />
            {isSearching && (
        ...
        <div ref={resultsRef} className="max-h-80 sm:max-h-96 overflow-y-auto">
          {!searchTerm || searchTerm.length < 2 ? (
            <div className="text-center py-8 sm:py-12">
              <Search size={48} className="text-gray-300 dark:text-gray-600 mx-auto mb-4 sm:w-16 sm:h-16" />
              <p className="text-gray-500 dark:text-gray-400 text-base sm:text-lg">ابدأ بكتابة كلمة للبحث في القرآن الكريم</p>
              <div className="text-gray-400 dark:text-gray-500 text-sm mt-2 space-y-1">
                <p>يمكنك البحث عن أي كلمة أو عبارة في النص القرآني</p>
                <p className="text-emerald-600 dark:text-emerald-400">✨ البحث يعمل بدون تشكيل - اكتب الكلمة بشكل عادي</p>
              </div>
            </div>
          ) : searchResults.length === 0 && !isSearching ? (
            <div className="text-center py-8 sm:py-12">
              <BookOpen size={48} className="text-gray-300 dark:text-gray-600 mx-auto mb-4 sm:w-16 sm:h-16" />
              <p className="text-gray-500 dark:text-gray-400 text-base sm:text-lg">لم يتم العثور على نتائج</p>
              <div className="text-gray-400 dark:text-gray-500 text-sm mt-2 space-y-1">
                <p>جرب البحث بكلمات أخرى أو تأكد من الإملاء</p>
                <p className="text-blue-600 dark:text-blue-400">💡 تذكر: يمكنك البحث بدون تشكيل</p>
              </div>
            </div>
          ) : (
                ...
                <div
                  ...
                  className={`p-3 sm:p-4 rounded-lg sm:rounded-xl border cursor-pointer transition-all duration-200 group active:scale-[0.98] ${
                    index === selectedIndex
                      ...
                      : 'bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:border-emerald-300 dark:hover:border-emerald-600'
                  }`}
                >
                  <div className="flex items-start justify-between gap-3 sm:gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                        ...
                        <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm text-emerald-700 min-w-0">
                          <span className="font-semibold truncate dark:text-emerald-300">{result.surahInfo.arabicName}</span>
                          <span className="hidden sm:inline">•</span>
                          <span className="whitespace-nowrap dark:text-emerald-300">الآية {result.verse.ayah}</span>
                          <span className="hidden sm:inline">•</span>
                          <span className="flex items-center gap-1 whitespace-nowrap">
                            <MapPin size={10} className="sm:w-3 sm:h-3" />
                            <span className="dark:text-emerald-300">صفحة {result.verse.page}</span>
                          </span>
                        </div>
                      </div>
                      ...
                      <div 
                        className="text-sm sm:text-lg leading-relaxed text-gray-800 dark:text-gray-200 mb-2 line-clamp-3"
                        dir="rtl"
                        dangerouslySetInnerHTML={{ __html: result.highlightedText }}
                      />
                      
                      {/* Additional Info */}
                      <div className="flex items-center gap-2 sm:gap-4 text-xs text-emerald-600 dark:text-emerald-400">
                    ...
                    <ArrowLeft 
                      size={16} 
                      className="text-emerald-500 dark:text-emerald-400 group-hover:text-emerald-700 dark:group-hover:text-emerald-300 group-hover:-translate-x-1 transition-all duration-200 flex-shrink-0 mt-2 sm:w-5 sm:h-5" 
                    />
                  </div>
                </div>
              ))}
